...
import { affiliateOffers, type AffiliateOffer } from "@/affiliate/affiliate-map";
...
const OFFER_COPY: Record<string, { headline: string; body: string; cta: string }> = {
  grammarly: {
...
export default function PostToolOffer({
  ...
  const offerKey = SLUG_OFFERS[toolSlug] ?? CATEGORY_OFFERS[toolCategory] ?? null;
  const offer: AffiliateOffer | null = offerKey ? affiliateOffers[offerKey] ?? null : null;
