...
export default function UsageStreakPrompt() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 flex justify-center px-4 pb-4 pointer-events-none">
      <div className="w-full max-w-2xl pointer-events-auto animate-[slideUp_0.4s_cubic-bezier(0.16,1,0.3,1)]">
        <div
        ...
        >
          ...
          <div className="px-5 py-4 flex flex-col sm:flex-row sm:items-center gap-4">
            {/* Badge */}
            <div
              className={`flex items-center gap-2.5 flex-shrink-0 ${isPowerUser ? "text-white" : "text-indigo-700"}`}
            >
              <div
              ...
              <div>
                <p
                  className={`text-[10px] font-black uppercase tracking-widest ${isPowerUser ? "text-violet-200" : "text-indigo-400"}`}
                >
                  {isPowerUser ? "Power User 🔥" : "On a roll ⚡"}
                </p>
                <p
                  className={`text-sm font-bold leading-snug ${isPowerUser ? "text-white" : "text-gray-900"}`}
                >
                  {isPowerUser
            ...
            {status === "success" ? (
              <div
                className={`flex items-center gap-2 text-sm font-semibold ${isPowerUser ? "text-white" : "text-emerald-600"}`}
              >
                <svg
                  className="w-4 h-4"
                  fill="none"
              ...
              >
                ...
                {status === "error" && (
                  <p
                    className={`text-xs ${isPowerUser ? "text-red-300" : "text-red-500"}`}
                  >
                    {errorMsg}
                  </p>
                )}
