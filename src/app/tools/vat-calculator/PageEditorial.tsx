...
function QRModal({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    import("qrcode").then((QRCode) => {
      ...
      QRCode.toCanvas(canvasRef.current, TOOL_URL, { width: 220, margin: 2, color: { dark: "#1e1b4b", light: "#eef2ff" } });
    });
...
function ShareBar() {
  ...
  const shareText = encodeURIComponent("Free VAT calculator — add or remove VAT from any price. UK 20%, 5%, or custom rate. Shows full net/VAT/gross breakdown. No signup.");
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-6 py-5 mb-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button onClick={copyLink} className="px-3 py-2 rounded-xl bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs font-semibold">{copied ? <span className="text-indigo-600">✓ Copied!</span> : "Copy link"}</button>
            <button onClick={() => setQrOpen(true)} className="hidden sm:block px-3 py-2 rounded-xl bg-gradient-to-r from-indigo-500 to-blue-600 text-white text-xs font-semibold hover:-translate-y-0.5 transition-all">Scan QR</button>
          </div>
        </div>
      </div>
    </>
...
const UK_VAT_RATES = [
  ["20%", "Standard rate", "Most goods and services — the default rate that applies unless a reduced or zero rate specifically applies."],
  ["5%",  "Reduced rate",  "Home energy (gas, electricity), children's car seats, contraceptive products, mobility aids, some renovation work."],
  ["0%",  "Zero rate",     "Most food (not restaurant meals), children's clothing, books and newspapers, public transport fares, prescription medicines."],
  ["Exempt", "VAT exempt", "Financial services, insurance, education, health services — VAT is not charged but cannot be reclaimed on costs."],
];
  ...
  return (
    <>
      ...
      <section id="how-to-use" className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <h2 className="text-4xl font-bold text-gray-900 mb-4 text-center">How to Use the VAT Calculator</h2>
        <p className="text-lg text-gray-500 text-center max-w-2xl mx-auto mb-14 leading-relaxed">Select Add or Remove VAT, enter your price, choose a rate, and get the net amount, VAT amount, and gross total instantly.</p>
        ...
        <div className="space-y-6 mb-14">
          {[
              ...
              enrich: (
                <div className="overflow-x-auto rounded-xl border border-gray-100 text-sm">
                  <table className="w-full text-left">
                    <thead className="bg-gray-50"><tr><th className="px-4 py-2 text-xs font-bold text-gray-500 uppercase">Mode</th><th className="px-4 py-2 text-xs font-bold text-gray-500 uppercase">Formula</th><th className="px-4 py-2 text-xs font-bold text-gray-500 uppercase">When to use</th></tr></thead>
                    <tbody className="divide-y divide-gray-50">
                      {[
                        ["Add VAT",    "Gross = Net × (1 + rate/100)",      "You have an ex-VAT price and need the VAT-inclusive total"],
                        ["Remove VAT", "Net = Gross ÷ (1 + rate/100)", "You have a VAT-inclusive total and need the net amount and VAT"],
                      ].map(([mode, formula, use]) => (
                        <tr key={mode} className="hover:bg-indigo-50">
                          <td className="px-4 py-2 font-bold text-indigo-700 text-xs whitespace-nowrap">{mode}</td>
                          <td className="px-4 py-2 text-xs font-mono text-gray-700">{formula}</td>
            ...
            },
            {
              ...
              enrich: <div className="bg-indigo-50 rounded-xl px-5 py-4 text-sm text-indigo-800 leading-relaxed"><strong>Which price to enter:</strong> If you're quoting a customer and your price list shows ex-VAT prices, use Add VAT. If you have a receipt or invoice showing a total that includes VAT and need to know the net cost for accounting or expense claims, use Remove VAT.</div>,
            },
            {
              n: 3,
              ...
              enrich: (
                <div className="overflow-x-auto rounded-xl border border-gray-100 text-sm">
                  <table className="w-full text-left">
                    <thead className="bg-gray-50"><tr><th className="px-4 py-2 text-xs font-bold text-gray-500 uppercase">UK Rate</th><th className="px-4 py-2 text-xs font-bold text-gray-500 uppercase">Rate name</th><th className="px-4 py-2 text-xs font-bold text-gray-500 uppercase">What it covers</th></tr></thead>
                    <tbody className="divide-y divide-gray-50">
                      {UK_VAT_RATES.map(([rate, name, desc]) => (
                        <tr key={rate} className="hover:bg-indigo-50">
                          <td className="px-4 py-2 font-bold text-indigo-700 text-xs whitespace-nowrap">{rate}</td>
                          <td className="px-4 py-2 font-semibold text-gray-900 text-xs">{name}</td>
            ...
            },
            {
              ...
              enrich: <div className="bg-blue-50 rounded-xl px-5 py-4 text-sm text-blue-800 leading-relaxed"><strong>For invoicing:</strong> When issuing a VAT invoice, you must show the net amount, the VAT rate, the VAT amount, and the gross total as separate line items. This tool gives you all four figures — copy them directly onto your invoice. UK VAT invoices must also show your VAT registration number.</div>,
            },
          ].map(({ n, title, body, enrich }) => (
            <div key={n} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-7 flex gap-5">
              <div className="flex-shrink-0 w-10 h-10 rounded-full bg-indigo-600 text-white font-black text-lg flex items-center justify-center">{n}</div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-14">
          {[
            { emoji: "🧾", title: "Invoicing clients", desc: "Calculate the VAT to add to your net price for client invoices — shows the exact VAT amount and gross total to include." },
            { emoji: "🛒", title: "Checking receipts", desc: "Remove VAT from a receipt total to find the net cost — useful for expense claims where you need to separate VAT from the cost." },
            { emoji: "📊", title: "Bookkeeping and accounts", desc: "Quickly separate net and VAT amounts from mixed totals for accurate bookkeeping entries and VAT return completion." },
            { emoji: "💰", title: "Price quoting", desc: "Convert your ex-VAT list prices to VAT-inclusive prices for customer quotes — or display both clearly on quotes." },
            { emoji: "🏗️", title: "Tradespeople and contractors", desc: "Add 20% VAT to labour and materials costs to produce the correct customer-facing quote total." },
            { emoji: "🌍", title: "EU and international rates", desc: "Use custom rate mode to calculate VAT at any European or international rate — enter 21%, 23%, 25%, or any other rate." },
          ].map(({ emoji, title, desc }) => (
            <div key={title} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 hover:border-indigo-200 hover:-translate-y-1 transition-all duration-200">
        ...
        <div className="bg-gradient-to-br from-indigo-600 to-blue-700 rounded-2xl p-8 text-white text-center mb-14">
          ...
          <h3 className="text-xl font-bold mb-3">Removing VAT is not the same as subtracting the VAT percentage — use the correct reverse formula</h3>
          <p className="text-indigo-100 leading-relaxed max-w-xl mx-auto text-sm">A common mistake when working with VAT-inclusive prices is to simply subtract the VAT rate percentage from the gross total. For 20% VAT, this appears to work by coincidence (£120 − £120×20% = £96, not £100), but the correct reverse calculation is to divide the gross by 1.20, giving £100. For other rates — 5%, 15%, 23% — the subtraction method gives significantly wrong answers. The correct formula for any rate is: Net = Gross ÷ (1 + rate/100). This calculator uses the correct formula in both modes.</p>
        </div>
        ...
        <div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              { href: "/tools/sales-tax-calculator", label: "Sales Tax Calculator", desc: "Calculate US sales tax — add or remove tax from a price using any state or local rate." },
              ...
              { href: "/tools/invoice-generator", label: "Invoice Generator", desc: "Create professional invoices with itemised billing, automatic totals, and PDF download." },
            ].map(({ href, label, desc }) => (
              <a key={href} href={href} className="block bg-white rounded-xl shadow-sm border-2 border-transparent hover:border-indigo-200 hover:-translate-y-1 transition-all duration-200 p-5">
