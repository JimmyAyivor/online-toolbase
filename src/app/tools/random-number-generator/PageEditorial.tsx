...
function ShareBar() {
  ...
  const shareText = encodeURIComponent("Free random number generator — generate numbers in any range, with optional no-duplicate mode. No signup.");
...
const USE_CASES_TABLE = [
  ...
  ["1–N (×K, unique)",  "Randomly ordering or sampling K items from a list of N"],
];
  ...
  return (
    <>
      ...
      <section id="how-to-use" className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <h2 className="text-4xl font-bold text-gray-900 mb-4 text-center">How to Use the Random Number Generator</h2>
        <p className="text-lg text-gray-500 text-center max-w-2xl mx-auto mb-14 leading-relaxed">Set your minimum, maximum, and how many numbers you want — enable unique mode if you need no repeats — then generate and copy.</p>
        ...
        <div className="space-y-6 mb-14">
          {[
              ...
              enrich: (
                <div className="overflow-x-auto rounded-xl border border-gray-100 text-sm">
                  <table className="w-full text-left">
                    <thead className="bg-gray-50"><tr><th className="px-4 py-2 text-xs font-bold text-gray-500 uppercase">Range</th><th className="px-4 py-2 text-xs font-bold text-gray-500 uppercase">Typical use</th></tr></thead>
                    <tbody className="divide-y divide-gray-50">
                      {USE_CASES_TABLE.map(([range, use]) => (
                        <tr key={range} className="hover:bg-indigo-50">
                          <td className="px-4 py-2 font-mono font-bold text-indigo-700 text-xs">{range}</td>
            ...
            },
            {
              ...
              enrich: <div className="bg-indigo-50 rounded-xl px-5 py-4 text-sm text-indigo-800 leading-relaxed"><strong>Unique mode:</strong> Enable 'No duplicate numbers' to ensure each number in your set appears only once. This is equivalent to sampling without replacement — like drawing numbered balls from a lottery drum and not putting them back. If you request more unique numbers than exist in your range (e.g. 10 unique numbers from a range of 1–5), the tool will show an error rather than producing duplicates.</div>,
            },
            {
              n: 3,
              ...
              enrich: <div className="bg-blue-50 rounded-xl px-5 py-4 text-sm text-blue-800 leading-relaxed"><strong>Statistics note:</strong> For large sets of numbers generated from a wide range, the average should converge towards the midpoint of your range as count increases — this is the expected value for a uniform distribution. If you generate 1,000 numbers from 1–100, the average will typically be very close to 50.5 (the exact midpoint). Smaller sets will show more variance, which is expected and correct.</div>,
            },
            {
              n: 4,
              ...
              enrich: <div className="bg-indigo-50 rounded-xl px-5 py-4 text-sm text-indigo-800 leading-relaxed"><strong>CSV format:</strong> The copied output is comma-separated (e.g. '42, 7, 93, 15') with a space after each comma for readability. Most spreadsheet applications (Excel, Google Sheets) will split this into individual cells when you use 'Paste Special → Split text to columns' with comma as the delimiter.</div>,
            },
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-14">
          {[
            { emoji: "🎲", title: "Games and dice rolling", desc: "Roll d4, d6, d8, d10, d12, d20, or any custom die — generate one or multiple rolls at once." },
            { emoji: "🎰", title: "Lottery number picks", desc: "Generate 6 unique numbers from 1–49 (UK Lotto), 5 from 1–70 (Powerball), or any other lottery format." },
            { emoji: "🏆", title: "Raffles and draws", desc: "Assign numbers to participants and generate a random winner — use unique mode to ensure one number per person." },
            { emoji: "📊", title: "Classroom and statistics", desc: "Generate random samples for probability exercises, teach expected value, or demonstrate distribution concepts." },
            { emoji: "🔀", title: "Random ordering", desc: "Generate K unique numbers from 1–N to create a random ordering or selection of items from a numbered list." },
            { emoji: "🎮", title: "Game development testing", desc: "Generate test values, random IDs, or sample datasets for development and testing purposes." },
        ...
        <div className="bg-gradient-to-br from-indigo-600 to-blue-700 rounded-2xl p-8 text-white text-center mb-14">
          ...
          <h3 className="text-xl font-bold mb-3">Pseudo-random is random enough for every everyday purpose — the distinction only matters for security-critical applications</h3>
          <p className="text-indigo-100 leading-relaxed max-w-xl mx-auto text-sm">The random numbers generated by this tool use JavaScript's Math.random() — a pseudo-random number generator (PRNG) that produces statistically uniform distributions and passes all standard randomness tests. For the vast majority of uses — games, raffles, classroom activities, sampling, creative decisions, and simulations — this is indistinguishable from 'true' randomness. The only contexts where it matters are cryptographic security (generating encryption keys or secure tokens) and certified gambling systems (regulated online casino games). For everything else, click generate and trust the result.</p>
        </div>
