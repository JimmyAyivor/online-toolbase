...
function ShareBar() {
  ...
  const shareText = encodeURIComponent("Free scientific calculator — trig, log, sqrt, powers, π, e, DEG/RAD modes, and history. No signup.");
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-6 py-5 mb-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button onClick={() => setQrOpen(true)} className="hidden sm:block px-3 py-2 rounded-xl bg-gradient-to-r from-slate-700 to-indigo-800 text-white text-xs font-semibold hover:-translate-y-0.5 transition-all">Scan QR</button>
          </div>
        </div>
      </div>
    </>
  ...
  return (
    <>
      ...
      <section id="how-to-use" className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <h2 className="text-4xl font-bold text-gray-900 mb-4 text-center">How to Use the Scientific Calculator</h2>
        <p className="text-lg text-gray-500 text-center max-w-2xl mx-auto mb-14 leading-relaxed">Build expressions using the function buttons, number pad, and operators — press = to evaluate. Use DEG or RAD mode for trigonometric functions.</p>
          ...
          {[
            {
              ...
              enrich: <div className="bg-indigo-50 rounded-xl px-5 py-4 text-sm text-indigo-800 leading-relaxed"><strong>Quick check:</strong> In DEG mode, sin(90) should equal 1 exactly. In RAD mode, sin(1.5708) should equal approximately 1 (since π/2 ≈ 1.5708). If your result is wrong, the first thing to check is whether you're in the correct angle mode.</div>,
            },
            {
              n: 2,
              ...
              enrich: (
                <div className="overflow-x-auto rounded-xl border border-gray-100 text-sm">
                  <table className="w-full text-left">
                    <thead className="bg-gray-50"><tr><th className="px-4 py-2 text-xs font-bold text-gray-500 uppercase">Function</th><th className="px-4 py-2 text-xs font-bold text-gray-500 uppercase">What it does</th><th className="px-4 py-2 text-xs font-bold text-gray-500 uppercase">Example</th></tr></thead>
                    <tbody className="divide-y divide-gray-50">
                      {FUNCTIONS_TABLE.map(([fn, desc, ex]) => (
                        <tr key={fn} className="hover:bg-indigo-50">
                          <td className="px-4 py-2 font-mono font-bold text-indigo-700 text-xs">{fn}</td>
                          <td className="px-4 py-2 text-xs text-gray-700">{desc}</td>
                          <td className="px-4 py-2 text-xs font-mono text-gray-500">{ex}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            },
            {
              ...
              enrich: <div className="bg-slate-50 rounded-xl px-5 py-4 text-sm text-slate-700 leading-relaxed"><strong>Parentheses tip:</strong> When in doubt, use more parentheses. A well-parenthesised expression like (2+3)*(4-1) is always clearer and safer than 2+3*4-1 (which evaluates as 2+(3*4)-1 = 13, not as 5*3 = 15). Functions like sin( already include an opening parenthesis — remember to close it with the ) button after your argument.</div>,
            },
            {
              n: 4,
              ...
              enrich: <div className="bg-indigo-50 rounded-xl px-5 py-4 text-sm text-indigo-800 leading-relaxed"><strong>Multi-step calculations:</strong> For problems requiring multiple steps (e.g. solving a quadratic, calculating compound interest), work through each step individually and note intermediate results from the History panel. Each step's result can be the starting point for the next expression — clear the display with C and enter the next stage of your calculation.</div>,
            },
          ].map(({ n, title, body, enrich }) => (
            <div key={n} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-7 flex gap-5">
              <div className="flex-shrink-0 w-10 h-10 rounded-full bg-indigo-700 text-white font-black text-lg flex items-center justify-center">{n}</div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-14">
          {[
            { emoji: "📐", title: "Trigonometry homework", desc: "Calculate sin, cos, and tan values for angles in both degrees and radians — use DEG mode for school geometry, RAD for calculus." },
            { emoji: "📊", title: "Physics calculations", desc: "Work with logarithms, exponentials, and trig functions for physics problems — switch to RAD mode for angular velocity and wave calculations." },
            { emoji: "🧮", title: "Engineering computations", desc: "Evaluate expressions involving powers, roots, logarithms, and constants π and e for engineering and science coursework." },
            { emoji: "💹", title: "Financial mathematics", desc: "Calculate compound interest (using e^rt for continuous compounding), logarithmic growth rates, and power functions." },
            { emoji: "🎓", title: "Exam preparation", desc: "Practice scientific calculations ahead of maths, physics, or chemistry exams — the layout mirrors physical scientific calculators." },
            { emoji: "🔬", title: "Scientific notation", desc: "Work with large and small numbers using powers of 10 — calculate 10^-9 for nanometres, 6.022*10^23 for Avogadro's number." },
        ...
        <div className="bg-gradient-to-br from-slate-700 to-indigo-800 rounded-2xl p-8 text-white text-center mb-14">
          ...
          <h3 className="text-xl font-bold mb-3">DEG vs RAD — the most common source of wrong answers in trigonometry</h3>
          <p className="text-slate-300 leading-relaxed max-w-xl mx-auto text-sm">The single most common cause of incorrect trigonometric results is being in the wrong angle mode. sin(90) in DEG mode correctly returns 1. sin(90) in RAD mode returns approximately 0.894 — because 90 radians is roughly 5156 degrees, not 90 degrees. Most students and professionals work in degrees for everyday calculations; most programming languages and advanced mathematics use radians by default. Before using sin, cos, or tan, always verify which mode you're in and which mode your problem requires. This calculator defaults to DEG mode — switch to RAD when your problem specifies radian angles or when using the calculator for calculus and physics computations.</p>
        </div>
        ...
        <div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              { href: "/tools/fraction-calculator", label: "Fraction Calculator", desc: "Add, subtract, multiply, and divide fractions with step-by-step working shown." },
              ...
              { href: "/tools/unit-converter", label: "Unit Converter", desc: "Convert between hundreds of units across length, weight, volume, temperature, and more." },
            ].map(({ href, label, desc }) => (
