...
function QRModal({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => { let c = false; import("qrcode").then((Q) => { if (c || !canvasRef.current) return; Q.toCanvas(canvasRef.current, TOOL_URL, { width: 220, margin: 2, color: { dark: "#1e3a5f", light: "#eff6ff" } }); }); return () => { c = true; }; }, []);
  return (
...
function ShareBar() {
  ...
  const shareText = encodeURIComponent("Free aspect ratio calculator — find ratio from dimensions, calculate missing width/height, scale proportionally. No signup.");
  ...
  return (
    <>
      ...
      <section id='how-to-use' className='max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10' aria-labelledby='how-to-use-heading'>
        <h2 id='how-to-use-heading' className='text-4xl font-bold text-gray-900 mb-4 text-center'>How to Use the Aspect Ratio Calculator</h2>
        <p className='text-lg text-gray-500 text-center max-w-2xl mx-auto mb-14 leading-relaxed'>Choose the mode that matches your need — find a ratio, calculate a missing dimension, or scale proportionally — then enter your values and copy the result.</p>
        <div className='space-y-6 mb-14'>
          {[
            { n: 1, title: "Find Ratio — discover the aspect ratio of any dimensions", body: "Enter any width and height in pixels (or any unit) to get the simplified aspect ratio, decimal equivalent, and megapixel count. This is useful when you have an image or video at an unusual resolution and want to know what standard ratio it corresponds to — for example, 2560×1600 simplifies to 8:5, which is the 16:10 widescreen monitor ratio.", enrich: <div className='bg-blue-50 rounded-xl px-5 py-4 text-sm text-blue-800 leading-relaxed'><strong>Common ratio preset:</strong> Click any of the 8 ratio buttons below the mode selector to instantly populate the inputs with standard dimensions at that ratio (e.g. clicking 16:9 enters 1600×900px). The visual preview updates to show the shape of the selected ratio.</div> },
            { n: 2, title: "Find Height / Find Width — calculate missing dimensions", body: "If you need an image at a specific width but don't know what height to use to maintain a given ratio — enter the width and the ratio (e.g. width 1200, ratio 16:9) to get the correct height (675). This is the most common use case for resizing images for specific platforms without distortion.", enrich: <div className='bg-indigo-50 rounded-xl px-5 py-4 text-sm text-indigo-800 leading-relaxed'><strong>Example:</strong> You're creating a YouTube thumbnail (16:9) but you know you need it to be 1280px wide. Enter width: 1280, ratio: 16:9 → result: height 720px (1280×720 = 720p HD). Use 'Find Width' for the reverse: enter height 1080, ratio 16:9 → width 1920.</div> },
            { n: 3, title: "Scale — resize proportionally by percentage", body: "Enter original dimensions and a scale percentage to get the new width and height while keeping the exact same ratio. 50% gives you half the size; 200% doubles it. Useful for creating responsive image sets (1x, 2x, 3x) or downscaling large source files to web-appropriate sizes.", enrich: <div className='bg-blue-50 rounded-xl px-5 py-4 text-sm text-blue-800 leading-relaxed'><strong>Responsive images:</strong> Start with your largest source image (e.g. 3840×2160) and scale to 50% for a 1920×1080 medium, and 25% for a 960×540 small. All three maintain the exact 16:9 ratio and can be used in HTML srcset attributes for responsive image serving.</div> },
            { n: 4, title: "Copy the result and use in your project", body: "Click the Copy button on the result panel to copy the dimensions to your clipboard. The copied format is 'width × height (ratio)' — for example '1920 × 1080 (16:9)'. Use the resolution reference table on the right to quickly identify standard resolutions and their corresponding aspect ratios without needing to calculate them.", enrich: <div className='bg-indigo-50 rounded-xl px-5 py-4 text-sm text-indigo-800 leading-relaxed'><strong>Visual preview:</strong> The blue rectangle below the result updates to visually represent the selected ratio. This is useful for immediately seeing whether a ratio is portrait, landscape, square, or ultra-wide — and helps catch errors where a dimension was entered in the wrong order.</div> },
          ].map(({ n, title, body, enrich }) => (
            <div key={n} className='bg-white rounded-2xl shadow-sm border border-gray-100 p-7 flex gap-5'>
              <div className='flex-shrink-0 w-10 h-10 rounded-full bg-blue-600 text-white font-black text-lg flex items-center justify-center'>{n}</div>
        ...
        <div className='grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-14'>
          {[
            { emoji: "🎬", title: "Video production", desc: "Verify export dimensions match 16:9 for YouTube, 9:16 for Reels, or 21:9 for cinematic cropping before rendering." },
            { emoji: "🖼️", title: "Graphic design", desc: "Find the correct height for a banner at a known width, or check whether a client's supplied image matches the required ratio." },
            { emoji: "📱", title: "Social media assets", desc: "Calculate correct dimensions for Instagram posts (4:5), Stories (9:16), LinkedIn headers (8:1), and Twitter/X banners (3:1)." },
            { emoji: "🖥️", title: "Web and UI design", desc: "Scale component dimensions proportionally, or find the correct CSS aspect-ratio value from pixel dimensions." },
            { emoji: "📷", title: "Photography", desc: "Check image crop ratios before printing, verify sensor format ratios (3:2, 4:3, 1:1), or plan crop dimensions." },
            { emoji: "🎮", title: "Game and app development", desc: "Calculate resolution variants for different device aspect ratios (iPhone 16:9, modern smartphones 19.5:9, tablets 4:3)." },
        ...
        <div className='bg-gradient-to-br from-blue-700 to-indigo-800 rounded-2xl p-8 text-white text-center mb-14'>
          ...
          <h3 className='text-xl font-bold mb-3'>Always maintain ratio when scaling — stretching or squashing an image is immediately visible and looks unprofessional</h3>
          <p className='text-blue-100 leading-relaxed max-w-xl mx-auto text-sm'>Distorted images — where width and height have been scaled independently — look immediately wrong to any viewer even if they can't explain why. Circles become ovals, faces look wider or taller than they should, and text becomes blurry or difficult to read. The single most common cause of distorted images in web and print production is resizing width and height independently without checking that the ratio is preserved. This calculator removes the mental arithmetic of maintaining proportions — enter one dimension and the ratio, and the correct corresponding dimension is calculated for you.</p>
        </div>
        <div>
          <h3 className='text-lg font-bold text-gray-900 mb-4'>Related Free Calculator Tools</h3>
          <div className='grid sm:grid-cols-2 lg:grid-cols-3 gap-4'>
            {[
              { href: "/tools/image-format-converter", label: "Image Format Converter", desc: "Convert between JPG, PNG, and WebP formats with quality control and file size comparison." },
              { href: "/tools/profile-picture-resizer", label: "Profile Picture Resizer", desc: "Resize profile photos to exact platform dimensions for Instagram, LinkedIn, Twitter, and more." },
              { href: "/tools/favicon-generator", label: "Favicon Generator", desc: "Generate favicons in all standard sizes from any image — ICO, PNG, and webmanifest." },
