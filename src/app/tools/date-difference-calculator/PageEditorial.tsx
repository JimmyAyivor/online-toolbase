...
function QRModal({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    import("qrcode").then((QRCode) => {
      ...
      QRCode.toCanvas(canvasRef.current, TOOL_URL, { width: 220, margin: 2, color: { dark: "#1e3a5f", light: "#eff6ff" } });
    });
...
function ShareBar() {
  ...
  const shareText = encodeURIComponent("Free date difference calculator — days, weeks, months, workdays between any two dates. Instant results, no signup.");
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-6 py-5 mb-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button onClick={copyLink} className="px-3 py-2 rounded-xl bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs font-semibold">{copied ? <span className="text-blue-600">✓ Copied!</span> : "Copy link"}</button>
            <button onClick={() => setQrOpen(true)} className="hidden sm:block px-3 py-2 rounded-xl bg-gradient-to-r from-blue-500 to-indigo-600 text-white text-xs font-semibold hover:-translate-y-0.5 transition-all">Scan QR</button>
          </div>
        </div>
      </div>
    </>
  ...
  return (
    <>
      ...
      <section id="how-to-use" className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <h2 className="text-4xl font-bold text-gray-900 mb-4 text-center">How to Use the Date Difference Calculator</h2>
        <p className="text-lg text-gray-500 text-center max-w-2xl mx-auto mb-14 leading-relaxed">Select a start date and end date, and the calculator instantly shows the difference in days, weeks, months, years, hours, minutes, workdays, and weekend days.</p>
          ...
          {[
            {
              ...
              enrich: <div className="bg-blue-50 rounded-xl px-5 py-4 text-sm text-blue-800 leading-relaxed"><strong>Tip:</strong> If you don't know the exact date, use the quick preset buttons below the inputs — 'Last 30 days', 'Last 90 days', 'Last 365 days', and 'Next 30 days' will automatically fill in the correct dates for each common range. These are useful for quickly checking how many workdays fall in a recent or upcoming period.</div>,
            },
            {
              n: 2,
              ...
              enrich: (
                <div className="overflow-x-auto rounded-xl border border-gray-100 text-sm">
                  <table className="w-full text-left">
                    <thead className="bg-gray-50"><tr><th className="px-4 py-2 text-xs font-bold text-gray-500 uppercase">Metric</th><th className="px-4 py-2 text-xs font-bold text-gray-500 uppercase">Definition</th><th className="px-4 py-2 text-xs font-bold text-gray-500 uppercase">Common use</th></tr></thead>
                    <tbody className="divide-y divide-gray-50">
                      {[
                        ["Days",         "Total calendar days including weekends",     "Contract deadlines, age calculation"],
                        ["Weeks",        "Complete 7-day weeks",                        "Project timelines, subscription durations"],
                        ["Months",       "Calendar months (not exact 30-day units)",   "Lease lengths, employment periods"],
                        ["Years",        "Complete calendar years",                     "Age, anniversary, tenure calculation"],
                        ["Workdays",     "Monday–Friday days only",                     "Business deadlines, HR notice periods"],
                        ["Weekend days", "Saturday–Sunday days only",                   "Staffing, availability planning"],
                        ["Hours",        "Total elapsed hours",                         "Billing, rental, energy calculations"],
                        ["Minutes",      "Total elapsed minutes",                       "Software timestamps, time tracking"],
                      ].map(([metric, def, use]) => (
                        <tr key={metric} className="hover:bg-blue-50">
                          <td className="px-4 py-2 font-bold text-blue-700 text-xs whitespace-nowrap">{metric}</td>
                          <td className="px-4 py-2 text-xs text-gray-700">{def}</td>
                          <td className="px-4 py-2 text-xs text-gray-500">{use}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            },
            {
              ...
              enrich: <div className="bg-indigo-50 rounded-xl px-5 py-4 text-sm text-indigo-800 leading-relaxed"><strong>Practical example:</strong> If you're wondering how many working days fell in Q1 of the current year, set the start date to January 1 and the end date to March 31 — the Workdays count will tell you the number of Monday-to-Friday days in that quarter (typically 64–66 depending on the year). Subtract any bank holidays manually for your specific country's working day count.</div>,
            },
            {
              n: 4,
              ...
              enrich: <div className="bg-blue-50 rounded-xl px-5 py-4 text-sm text-blue-800 leading-relaxed"><strong>No bank holidays:</strong> The workday count excludes Saturdays and Sundays but does not account for public holidays, bank holidays, or local non-working days. These vary by country and region — subtract the relevant holidays from the workday count manually if you need a precise business-day calculation for contractual or legal purposes.</div>,
            },
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-14">
          {[
            { emoji: "📅", title: "Project deadline planning", desc: "Calculate how many calendar days or workdays remain until a project deadline — or how many days a past project took." },
            { emoji: "👔", title: "Employment and HR", desc: "Calculate notice periods, employment tenure, probation lengths, and contract durations in days, weeks, or months." },
            { emoji: "🎂", title: "Age and anniversary", desc: "Calculate exact age in days, weeks, and years — or how many days until or since an anniversary or birthday." },
            { emoji: "⚖️", title: "Legal and contractual", desc: "Calculate contract durations, limitation periods, and deadline compliance in calendar days or business days." },
            { emoji: "💰", title: "Financial periods", desc: "Calculate the number of days in a billing period, interest accrual period, or investment holding period." },
            { emoji: "🗓️", title: "Event planning", desc: "Count down days to an event, calculate how long a past event lasted, or figure out how many weekends remain in a period." },
          ].map(({ emoji, title, desc }) => (
            <div key={title} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 hover:border-blue-200 hover:-translate-y-1 transition-all duration-200">
        ...
        <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl p-8 text-white text-center mb-14">
          ...
          <h3 className="text-xl font-bold mb-3">Calendar days and business days are different things — always clarify which applies to your deadline</h3>
          <p className="text-blue-100 leading-relaxed max-w-xl mx-auto text-sm">A common source of confusion in contracts, employment agreements, and project schedules is whether deadlines are expressed in calendar days (every day including weekends) or business days (Monday–Friday only). A '10 calendar day' deadline starting on a Monday falls the following Wednesday. A '10 business day' deadline starting on the same Monday falls on the second Friday (two weeks later). For legal and contractual deadlines, always confirm which type of day is specified — 'days' in legal documents typically means calendar days unless explicitly stated as business days. This calculator shows both counts so you can verify whichever applies to your specific situation.</p>
        </div>
        ...
        <div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              { href: "/tools/timestamp-converter", label: "Timestamp Converter", desc: "Convert Unix timestamps to readable dates and times — convert between formats instantly." },
              { href: "/tools/countdown-timer", label: "Countdown Timer", desc: "Count down to any future date or event — set a target date and see days, hours, and minutes remaining." },
              { href: "/tools/age-calculator", label: "Age Calculator", desc: "Calculate exact age in years, months, and days from a birth date to today or any specific date." },
            ].map(({ href, label, desc }) => (
              <a key={href} href={href} className="block bg-white rounded-xl shadow-sm border-2 border-transparent hover:border-blue-200 hover:-translate-y-1 transition-all duration-200 p-5">
