  ...
  return (
    <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm' onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className='relative bg-white rounded-3xl shadow-2xl p-8 mx-4 max-w-xs w-full text-center'>
        ...
        <div className='inline-block rounded-2xl overflow-hidden border-4 border-indigo-100 shadow-inner mb-5'><canvas ref={canvasRef} /></div>
...
function ShareBar() {
  ...
  const shareText = encodeURIComponent("Free content calendar planner — schedule social media posts across all platforms, weekly view, CSV export. No signup.");
        ...
        {FAQS.map((faq, i) => (
          <div key={i} className='border border-gray-100 rounded-xl overflow-hidden'>
            <button className='w-full text-left px-5 py-4 flex items-center justify-between gap-4 hover:bg-gray-50 transition-colors' onClick={() => setOpen(open === i ? null : i)} aria-expanded={open === i}>
              ...
              <span className='text-indigo-600 text-lg shrink-0'>{open === i ? "−" : "+"}</span>
            </button>
  ...
  return (
    <>
      ...
      <section id='how-to-use' className='max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10' aria-labelledby='how-to-use-heading'>
        <h2 id='how-to-use-heading' className='text-4xl font-bold text-gray-900 mb-4 text-center'>How to Use the Content Calendar Planner</h2>
        <p className='text-lg text-gray-500 text-center max-w-2xl mx-auto mb-14 leading-relaxed'>Plan your content week by week — add posts by platform, date, and type, see everything in a colour-coded weekly grid, and export your schedule as a CSV.</p>

        <div className='space-y-6 mb-14'>
          <div className='bg-white rounded-2xl shadow-sm border border-gray-100 p-7 flex gap-5'>
            <div className='flex-shrink-0 w-10 h-10 rounded-full bg-indigo-600 text-white font-black text-lg flex items-center justify-center'>1</div>
            <div>
              <h3 className='text-lg font-bold text-gray-900 mb-2'>Select a platform and content type</h3>
              <p className='text-gray-600 leading-relaxed mb-3'>Choose the social media platform your post is for (Instagram, TikTok, Twitter/X, LinkedIn, YouTube, or Facebook) and select the content type — post, story, reel, video, thread, or article. These two selections determine how the post appears in the calendar grid and how it's categorised in the exported CSV. Each platform is colour-coded in the weekly view so you can see your platform distribution at a glance.</p>
              <div className='overflow-x-auto rounded-xl border border-gray-100 text-sm'>
                <table className='w-full text-left'>
                  <thead className='bg-gray-50'><tr><th className='px-4 py-2 text-xs font-bold text-gray-500 uppercase'>Platform</th><th className='px-4 py-2 text-xs font-bold text-gray-500 uppercase'>Recommended frequency</th><th className='px-4 py-2 text-xs font-bold text-gray-500 uppercase'>Best content types</th></tr></thead>
                  <tbody className='divide-y divide-gray-50'>
                    {PLATFORM_GUIDE.map(([platform, freq, secondary, extra]) => (
                      <tr key={platform} className='hover:bg-indigo-50'>
                        <td className='px-4 py-2 font-bold text-indigo-700 text-xs whitespace-nowrap'>{platform}</td>
                        <td className='px-4 py-2 text-xs text-gray-700'>{freq}</td>
                        <td className='px-4 py-2 text-xs text-gray-500'>{secondary} · {extra}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className='bg-white rounded-2xl shadow-sm border border-gray-100 p-7 flex gap-5'>
            <div className='flex-shrink-0 w-10 h-10 rounded-full bg-indigo-600 text-white font-black text-lg flex items-center justify-center'>2</div>
            <div>
              <h3 className='text-lg font-bold text-gray-900 mb-2'>Set the date and add a caption or note</h3>
              <p className='text-gray-600 leading-relaxed mb-3'>Select the date you plan to publish this post. Add your caption, post concept, or brief notes in the caption field — this is your planning note rather than a final caption, so bullet points or a rough idea are fine. Adding context here makes the exported CSV useful as a brief for yourself or a team member who'll create the actual content.</p>
              <div className='bg-blue-50 rounded-xl px-5 py-4 text-sm text-blue-800 leading-relaxed'><strong>Planning vs writing:</strong> Use the caption field for planning-level notes rather than final copy — "Reel: 3 productivity hacks for remote workers, upbeat music, text overlay" is more useful at the planning stage than a full caption. You can always write the final caption in your scheduling tool (Buffer, Later, etc.) when it's time to create the post.</div>
            </div>
          </div>

          <div className='bg-white rounded-2xl shadow-sm border border-gray-100 p-7 flex gap-5'>
            <div className='flex-shrink-0 w-10 h-10 rounded-full bg-indigo-600 text-white font-black text-lg flex items-center justify-center'>3</div>
            <div>
              <h3 className='text-lg font-bold text-gray-900 mb-2'>Review your weekly calendar grid</h3>
              <p className='text-gray-600 leading-relaxed mb-3'>Once you've added posts, the weekly calendar grid shows all planned content colour-coded by platform. Use this view to check your posting distribution — are you posting consistently across the week, or clustered on certain days? Is your platform mix balanced? Are there gaps where you've planned nothing, or days where you've over-scheduled? Adjust individual posts using the edit or delete controls.</p>
              <div className='bg-indigo-50 rounded-xl px-5 py-4 text-sm text-indigo-800 leading-relaxed'><strong>Balanced distribution tip:</strong> Aim to spread posts evenly across weekdays rather than clustering on Monday or Friday. Midweek posts (Tuesday–Thursday) typically see higher engagement across most platforms because weekend content has lower competition and weekday mornings capture commute-time browsing. Avoid scheduling multiple posts on the same platform on the same day unless your audience and strategy specifically support it.</div>
            </div>
          </div>

          <div className='bg-white rounded-2xl shadow-sm border border-gray-100 p-7 flex gap-5'>
            <div className='flex-shrink-0 w-10 h-10 rounded-full bg-indigo-600 text-white font-black text-lg flex items-center justify-center'>4</div>
            <div>
              <h3 className='text-lg font-bold text-gray-900 mb-2'>Export your schedule as CSV</h3>
              <p className='text-gray-600 leading-relaxed mb-3'>Click "Export CSV" to download your entire content schedule as a spreadsheet-compatible file. The CSV includes platform, content type, date, and caption for every planned post. Open it in Excel, Google Sheets, or Notion to share with your team, use as a brief for content creation, or import into a scheduling tool. The calendar data stays in your browser — nothing is saved to a server, so export before closing the tab if you want to keep your plan.</p>
              <div className='bg-blue-50 rounded-xl px-5 py-4 text-sm text-blue-800 leading-relaxed'><strong>Workflow tip:</strong> Export your CSV at the end of each planning session and save it to a shared folder (Google Drive, Dropbox) so your team always has access to the current content plan. Use one exported CSV per week or month, named clearly (e.g. "content-plan-jan-2025.csv") for easy version tracking.</div>
            </div>
          </div>
        </div>
        ...
        <h3 className='text-2xl font-bold text-gray-900 mb-6'>Common uses for content calendar planning</h3>
        <div className='grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-14'>
          {[
            { emoji: "📅", title: "Weekly content batching", desc: "Plan an entire week of content in one session — decide what to post where before sitting down to create, saving time and reducing daily decision fatigue." },
            { emoji: "👥", title: "Team content coordination", desc: "Share the exported CSV with writers, designers, and videographers so everyone knows what's due when and for which platform." },
            { emoji: "🚀", title: "Product or campaign launches", desc: "Map out a coordinated launch sequence across multiple platforms — teaser posts, launch day content, and follow-up posts all planned in advance." },
            { emoji: "📊", title: "Content mix analysis", desc: "Visualise your platform distribution and content type mix — ensure you're not over-posting on one platform or neglecting another." },
            { emoji: "🔄", title: "Repurposing workflow", desc: "Plan content repurposing sequences — a YouTube video on Tuesday feeds TikTok clips Wednesday–Friday, a LinkedIn article on Thursday, and Instagram carousel on Saturday." },
            { emoji: "🗓️", title: "Seasonal and event planning", desc: "Map content around key dates — holidays, industry events, product anniversaries, and awareness months — so you're never scrambling last minute." },
        ...
        <div className='bg-gradient-to-br from-indigo-700 to-blue-800 rounded-2xl p-8 text-white text-center mb-14'>
          ...
          <h3 className='text-xl font-bold mb-3'>A content calendar transforms posting from reactive to intentional — consistency compounds over time</h3>
          <p className='text-indigo-100 leading-relaxed max-w-xl mx-auto text-sm'>Most social media accounts with inconsistent growth share a common pattern: they post frequently when motivated and disappear for weeks when busy or uninspired. The accounts that grow consistently almost always have a planned content calendar — not because planned content is inherently better, but because a plan prevents the posting gaps that signal to algorithms that an account is inactive. Algorithms favour accounts that post on a predictable schedule. Planning 1–2 weeks ahead means you're never scrambling for ideas the morning a post is due, and the consistency this creates compounds: more consistent posting leads to better algorithmic distribution, which leads to faster audience growth, which creates more motivation to post consistently.</p>
        </div>
        ...
        <div>
          ...
          <div className='grid sm:grid-cols-2 lg:grid-cols-3 gap-4'>
            {[
              ...
              { href: "/tools/social-media-audit", label: "Social Media Audit Tool", desc: "22-point checklist across profile basics, content quality, engagement, and strategy — get an instant score." },
            ].map((link) => (
              <a key={link.href} href={link.href} className='block bg-white rounded-xl shadow-sm border-2 border-transparent hover:border-indigo-200 hover:-translate-y-1 transition-all duration-200 p-5' aria-label={`${link.label} — ${link.desc}`}>
