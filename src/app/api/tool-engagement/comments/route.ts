...
import {
  getIp, isRateLimited, isDuplicateContent,
...
export async function GET(req: NextRequest) {
  ...
  const comments = await query<{ id: string; name: string; body: string; created_at: string }>(
...
export async function POST(req: NextRequest) {
  ...
  const { slug, name, commentBody, _hp } = body as {
    slug?: string; name?: string; commentBody?: string; _hp?: string;
  };
