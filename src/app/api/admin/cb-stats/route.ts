...
export async function GET(req: NextRequest) {
  ...
  const toolRows = await query<{ product_key: string; tool_slug: string; clicks: string }>(
  ...
  const dailyRows = await query<{ day: string; product_key: string; clicks: string }>(
  ...
  for (const r of toolRows) {
    ...
    if (toolMap[r.product_key].length < 5) {
      toolMap[r.product_key].push({ tool: r.tool_slug, clicks: parseInt(r.clicks, 10) });
    }
  }
